...
interface EventRow { id: string; title: string; description: string | null; event_date: string; attendee_count: number; is_registered: boolean }
...
export default function MentorEventsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try { setEvents((await api.get<{ events: EventRow[] }>('/api/mentorship/events')).events) }
  ...
  const toggle = async (ev: EventRow) => {
    ...
    try {
      ...
      await load()
    } catch (e) { setError(e instanceof Error ? e.message : 'Could not update your registration') }
    ...
    <MentorshipShell topBar={topBar}>
      <div className="p-6 max-w-5xl mx-auto">
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-500 mt-1">Host or attend platform-wide events.</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="panel-premium p-10 text-center text-sm text-gray-500">No upcoming events yet.</div>
        ) : (
          <div className="space-y-3">
            {events.map((ev) => (
              <div key={ev.id} className="panel-premium p-5 flex items-center justify-between gap-4 flex-wrap">
                ...
                <button onClick={() => toggle(ev)} disabled={busy === ev.id} className={`text-sm px-4 py-2 rounded-lg transition disabled:opacity-50 ${ev.is_registered ? 'btn-secondary' : 'btn-primary'}`}>
