...
export default function MentorProgramsPage() {
  ...
  const join = async (id: string) => {
    setBusy(id)
    try {
      await api.post(`/api/mentorship/programs/${id}/join`, { role: 'mentor' })
      await load()
    } catch (e) {
  ...
  const leave = async (id: string) => { setBusy(id); try { await api.delete(`/api/mentorship/programs/${id}/leave`); await load() } finally { setBusy(null) } }
    ...
    <MentorshipShell topBar={topBar}>
      <div className="p-6 max-w-5xl mx-auto">
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-500 mt-1">Volunteer to guide a cohort through a multi-session program.</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="panel-premium p-10 text-center text-sm text-gray-500">No programs open right now.</div>
        ) : (
          <div className="grid sm:grid-cols-2 gap-4">
            {programs.map((p) => (
              <div key={p.id} className="panel-premium p-5">
                ...
                {p.my_role === 'mentor' ? (
                  <button onClick={() => leave(p.id)} disabled={busy === p.id} className="btn-secondary mt-4 w-full text-sm py-2 disabled:opacity-50">{busy === p.id ? '…' : 'Leave Program'}</button>
                ) : (
                  <button onClick={() => join(p.id)} disabled={busy === p.id} className="btn-primary mt-4 w-full text-sm py-2 disabled:opacity-50">{busy === p.id ? '…' : 'Join as Mentor'}</button>
                )}
              </div>
            ))}
          </div>
